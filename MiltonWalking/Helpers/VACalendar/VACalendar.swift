...
protocol VACalendarDelegate: class {
    ...
    func assignedDaysDidUpdate(_ days: [VADay])
}
...
public class VACalendar {
    ...
    private var assignedDays = [VADay]() {
        didSet {
            delegate?.assignedDaysDidUpdate(selectedDays)
        }
    }
    ...
    public init(
        ...
        let startDate = startDate ?? calendar.date(byAdding: .year, value: 0, to: Date())!
        let endDate = endDate ?? calendar.date(byAdding: .year, value: 5, to: Date())!
    ...
    func assignedDates(_ dates: [Date]) {
        let days = months.flatMap { $0.days(for: dates) }
        days.forEach { $0.setSelectionState(.assigned) }
        assignedDays = days
    }
