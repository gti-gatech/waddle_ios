...
class VAMonth {
    ...
    private func state(for date: Date) -> VADayState {
        let cdate = Date()
        let result = date.compare(cdate)

        if date.year == cdate.year && date.month == cdate.month && date.dayOfMonth == cdate.dayOfMonth {
            return .selected
        }
        ...
        if (date.nameOfDay == "Saturday") || (date.nameOfDay == "Sunday") || (result == .orderedAscending) {
            return .out
        }
        
        if cdate.isSameAs(date) && calendar.isDate(date, equalTo: lastMonthDay, toGranularity: .month){
            return .available
        }
        if selectedDays.contains(where: { calendar.isDate($0.date , inSameDayAs: date) }) {
            return .selected
        }
        return .available
    }
