...
public protocol VACalendarViewDelegate: class {
    ...
    @objc optional func assignedDates(_ dates: [Date])
}
...
public class VACalendarView: UIScrollView {
    ...
    public func assignedDates(_ dates: [Date]) {
        calendar.unAssignAll()
        calendar.assignedDates(dates)
    }
...
extension VACalendarView: VACalendarDelegate {
    ...
    func assignedDaysDidUpdate(_ days: [VADay]) {
        let dates = days.map { $0.date }
        calendarDelegate?.assignedDates?(dates)
    }
}
...
extension VACalendarView: VAMonthViewDelegate {
    ...
    func dayStateChanged(_ day: VADay, in month: VAMonth) {
        ...
        case .single:
            guard day.state == .available || day.state == .assigned  else { return }
            calendar.deselectAll()
